 ...
 * Many bugs have been fixed in Torch and TF loaders, as well as in some processing layers. Now we check that on a certain set of networks results from OpenCV DNN match or very close to the results from the original frameworks. We also check that the results claimed in the papers for such networks are achievable with OpenCV DNN.
