 ...
 * Many bugs in Torch and TF loaders, as well as some layers have been fixed. Now we check that on a certain set of networks results from OpenCV DNN match the results from the original frameworks.
 * Performance has been substantially improved and is now on par with Intel Caffe, TensorFlow and other optimized frameworks. Layer fusion has been implemented and some performance-critical layers have been optimized using AVX, AVX2, SSE and NEON. An external BLAS (OpenBLAS, MKL, ATLAS) is not needed anymore.
 * New samples have been added. See https://github.com/opencv/opencv/tree/master/samples/dnn.
 * The optional Halide-based (see http://halide-lang.org) backend has been added. It can accelerate OpenCV DNN on GPU when the GPU is fast enough.

See https://github.com/opencv/opencv/wiki/Deep-Learning-in-OpenCV for details about the module.

p((((. !images/Intel_IPP_logo.png!

* Upgraded IPPICV from 2015.12 to 2017.2 version brought ~15% improvement in core and imgproc modules (measured as geometrical mean over the corresponding performance tests).

p((((. !images/speed.jpg!

* Dynamic dispatching of SSE4.2/AVX/AVX2 code has been implemented. Previously OpenCV had to be built with SSE4.x/AVX/AVX2 support in order to use such optimizations and that made it incompatible with older hardware. Now the OpenCV binaries automatically adapt to the real hardware and make use of new instructions if they are available while retaining compatibility with older hardware. All the existing AVX/AVX2 optimizations in OpenCV have been refactored to use this technology. AVX acceleration of DNN also uses dynamic dispatching.

p((((. !images/cxx11.jpg!

* OpenCV can now be configured and built as C++ 11 library. Use ENABLE_CXX11 CMake flag. On some modern Linux distributions, like the latest Fedora, it's enabled by default.
* New features for C++ 11 users have been added:

<pre>
// 1. use parallel_for_ with lambda to compute Mandelbrot fractal
parallel_for_(Range(0, img.rows*img.cols),
[&](const Range& r) {
...
auto A = Mat_<double>({0, -1, 0, -1, 5, -1, 0, -1, 0}).
							reshape(1, 3);
...
p((((. !images/imediasdk.jpg!

* Support for hardware-accelerated video encoding/decoding using Intel GPUs through Intel Media SDK has been implemented (in the form of backends for `cv::VideoCapture` and `cv::VideoWriter`).
  * H.264 and MPEG1/2 encoding/decoding is available. No containers support yet, only plain video streams.
  * The linux kernel should be compiled with the acceleration turned on and the Media SDK should be installed. See https://github.com/Intel-Media-SDK.
...
p((((. !images/openvx.png! !images/hal.jpg!
...
p((((. !images/win10.png! !images/elcapitan.png! 
...
* Many people contributed their code in the form of "pull requests":https://github.com/opencv/opencv/pulls?direction=desc&page=1&sort=created&state=closed. Here are some of the most interesting contributions, that were included into 2.4 branch:
** ">100":http://code.opencv.org/projects/opencv/versions/11 reported problems have been resolved since 2.4.3
** Oscar Deniz "submitted":https://github.com/opencv/opencv/pulls?direction=desc&page=1&sort=created&state=closed smile detector and sample.
...
* Another our GSoC student, Alexander Mordvintsev, added several new exciting Python samples, including ASIFT, hand-written digit recognition, motion debluring and even the use of Amazon PiCloud. Check https://github.com/opencv/opencv/tree/master/samples/python2

p((((. !images/profile.jpg!

* Yet another GSoC student Attila Novak has trained 2 new cascades, for silverware and profile faces. Check https://github.com/opencv/opencv/tree/master/data/lbpcascades
...
* Yet another result from GSoC (that was a good year indeed!): Yuri Zemlyansky implemented "simple flow" optical flow algorithm; check https://github.com/opencv/opencv/tree/master/samples/cpp/simpleflow_demo.cpp.
...
* OpenCV has never been that social. We are at github now: http://github.com/opencv/opencv. Grab the latest versions from there, submit pull requests.
