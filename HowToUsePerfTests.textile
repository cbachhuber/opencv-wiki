h1. Running performance tests and analyzing the results
...
h2. Running the tests with run.py
...
* --perf_<any>=<value> and --gtest_<any>=<value> - these options are passed to each test without the modifications (exception is --gtest_output parameter).
...
<pre>
cd opencv/build
python ../modules/ts/misc/run.py . ../android/build
</pre>
...
<pre>
cd opencv/build
python ../modules/ts/misc/run.py . -t core
</pre>
...
<pre>
cd opencv/build
python ../modules/ts/misc/run.py . -t core,imgproc --gtest_filter=*C1*
</pre>
...
cd opencv/build
mkdir -p logs
python ../modules/ts/misc/run.py . -w ./logs
</pre>
...
cd opencv
mkdir -p logs
python modules/ts/misc/run.py android/build -w ./logs
</pre>
...
cd opencv
mkdir -p logs
python modules/ts/misc/run.py android/build --android_test_data_path .
</pre>

h2. How to update perf data.
...
python ./clean_regex.py nonfree.xml "<regex_matching_your_tests>"
# see that regex works as expected
python ./clean_regex.py nonfree.xml "<regex_matching_your_tests>" do
</pre>
...
python opencv/modules/ts/misc/run.py -t nonfree --check --perf_write_sanity
</pre>
...
<pre>
cd build
python opencv/modules/ts/misc/run.py -t nonfree --check
</pre>

h2. Viewing results with report.py
...
<pre>
cd opencv/build
python ../modules/ts/misc/report.py *.xml
</pre>
...
h2. Comparing results with summary.py
...
h2. Viewing dependency from parameters with chart.py
