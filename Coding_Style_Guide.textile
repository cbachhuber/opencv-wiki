...
* Implementation files have _.cpp_ extension
* Every file includes BSD-compatible license in the beginning. Note that now we do not place the whole license text, but just reference it.
...
* External function names and data type names are written in mixed case. Classes start with capital letter, functions start with small letter (unless they are named after an algorithm author). External macros are written in upper case.
...
* Do not use C++ 11 yet (upd. for OpenCV 4.0 and above do use C++ 11. However, if you plan to submit bug fixes that should propagate to OpenCV 3.x or 2.4.x, stick with C++ 98 standard).
* Do not use malloc/free, new/delete. Use @cv::Mat@, @std::vector@, @std::map@, @cv::AutoBuffer@, @cv::Ptr@ instead. Those classes handle the memory automatically.
* Provide GTest-based tests for your code.
* Provide documentation for your code as Doxygen comments. Tutorials and samples are welcome.