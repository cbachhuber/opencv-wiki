...
The document is a short guide on code style, used in OpenCV. The core libraries (@core@, @imgproc@, etc) are written in C++, so the document concerns only the codes written in C++.

h2. Files

All the functionality must be put into one or more _.cpp_ and _.hpp_ files into the appropriate "module of OpenCV":https://github.com/Itseez/opencv/tree/2.4/modules, or a new module should be created if the contributed functionality is a rather large piece of code, or if it does not fit any existing module.
...
* Documentation is put to @opencv/modules/<module_name>/doc@, into one of the existing files or a new file/chapter is added there. In the latter case the file name should be listed in the TOC (table of content) file @opencv/modules/<module_name>/doc/<module_name>.rst@
* Tests are put to @opencv/modules/<module_name>/test@. When a new test source file is added, rerun CMake. If the test needs some data, put it to the separate "top-level directory":http://github.com/Itseez/opencv_extra/tree/master/testdata. For @ml@ module the test data is put to @opencv_extra/testdata/ml@ subdirectory, for other modules - to @opencv_extra/testdata/cv@ subdirectory. Please, limit size of your test data files within a few megabytes, the smaller the better. If some existing test data can be used for your tests (like @lena.jpg@ etc.), please, re-use it.

h2. File Structure

Every source file, except for the samples, starts with BSD-compatible license, which template can be found "here":https://github.com/Itseez/opencv/blob/master/LICENSE. The extra/different copyright clauses are possible, as long as the license stays BSD-compatible.
...
The functionality must be well defined and non-redundant. The function should be easy embedded into different processing pipelines that use other OpenCV functions.
...
Functions should not use return value for signaling about critical errors, such as null pointers, division by zero, bad argument range, unsupported image format etc. Instead, they should throw an exception, an instance of @cv::Exception@ or its derivative class. On the other hand, it is recommended to use a return value for signalizing about quite normal run-time situations that can happen in a correctly working system (e.g. tracked object goes outside of the image etc.)
...
A consistent argument order is important because it becomes easier to remember the order and it helps programmer to avoid errors, connecting with wrong argument order. The usual order is: _*<input parameters>, <output parameters>, <flags & optional parameters>.*_
...
For example function and class declarations, take a look at http://github.com/Itseez/opencv/blob/master/modules/core/include/opencv2/core/core.hpp.

h4. Using @InputArray@, @OutputArray@ etc.

In 2.4 we introduced new "proxy" datatypes in OpenCV to support multiple array types simultaneously. For example, in some cases it's more convenient a point set as vector<Point3f>, in other cases - as a matrix. In some cases it's more convenient to store homography matrix as @Mat@, in other - as @Matx33f@. If a parameter of your function has type @Mat@, @Matx<>@, @vector<Point...>@, @vector<Mat>@ or @vector<vector<Point...> >@, please, consider using @InputArray@, @OutputArray@ etc. types instead.
...
To address those goals, we physically separate interface and implementation part of our C++ classes. That is, we only expose interfaces, i.e. classes without constructors, without data members and with all purely virtual methods. The actual implementation is put into classes derived from those interfaces. The actual construction of the class is done by an exposed function. It returns smart pointer to the interface. There can be multiple "constructor" or "factory" functions, not necessarily placed in the same module. Users can add their own implementations of the same interface and provide the corresponding constructor functions. See @calib3d@ "module":https://github.com/Itseez/opencv/blob/master/modules/calib3d/include/opencv2/calib3d.hpp and the stereo correspondence algorithms (stereo matchers) as example of such classes.
...
* Also, add the constructor function that will return smart pointer to your base class:

<pre>CV_EXPORTS Ptr<MyStereoMatcher> createMyStereoMatcher(<params...>);</pre>

The constructor name follows @create<classname>[optional_suffix]@ pattern. There can be several constructors if you want.
...
The documentation for contributed functions is written in "ReStructured Text":http://openalea.gforge.inria.fr/doc/openalea/doc/_build/html/source/sphinx/rest_syntax.html and built using tool. To build documentation, you need a full Live Tex or compatible distribution installed (e.g. texlive-full on Ubuntu, "MacTex":http://sphinx.pocoo.org/ on Mac, MiKTeX on Windows) and Sphinx, which can be installed with @easy_install@ utility (@easy_install -U Sphinx@).
...
* the full description with short use samples (or links to the big complete samples in OpenCV repository), references to papers, formulas, etc

h2. Implementing tests

* For tests we use "GoogleTest":http://code.google.com/p/googletest/ framework. Please, check the documentation at the project site.
...
* The code is put into cv or nested namespace. Test code is put into cvtest namespace.
...
* Provide documentation for your code in RST format. Doxygen comments do not count (yet).